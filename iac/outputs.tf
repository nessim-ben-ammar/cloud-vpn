output "instance_public_ip" {
  description = "Public IP address of the cloud VPN instance"
  value       = var.assign_elastic_ip ? aws_eip.static_ip[0].public_ip : aws_instance.cloud_vpn_instance.public_ip
}

# Output SSH connection command
output "ssh_connection_command" {
  description = "Command to connect to the instance via SSH"
  value       = "ssh -i ${local_file.ssh_private_key.filename} ubuntu@${var.assign_elastic_ip ? aws_eip.static_ip[0].public_ip : aws_instance.cloud_vpn_instance.public_ip}"
}

output "vpn_endpoint" {
  description = "DNS name or IP clients should connect to"
  value = var.deploy_global_accelerator ? aws_globalaccelerator_accelerator.cloud_vpn[0].dns_name : (
    var.assign_elastic_ip ? aws_eip.static_ip[0].public_ip : aws_instance.cloud_vpn_instance.public_ip
  )
}
